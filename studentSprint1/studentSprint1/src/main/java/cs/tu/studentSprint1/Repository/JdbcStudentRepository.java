...
public class JdbcStudentRepository implements StudentInterface{
    ...
    @Override
    public List<Student> findApproveAll() {
        try {
            String sqlStudent = "SELECT display_th FROM Student";
            List<Student> students = jdbcTemplate.query(sqlStudent, new BeanPropertyRowMapper<>(Student.class));
            System.out.println(students);
            return students;
        } catch (IncorrectResultSizeDataAccessException e) {
            return null;
